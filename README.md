...
默认视觉 IP 是 Aiko，识别点为 **“蓝白机器人”**：黑色长直发、白色 T 恤、天蓝色下装，以及机器人袖章或腰侧挂件。Aiko 不是贴纸，也不是站在角落里的装饰物，而是正在认真参与系统运转的创作者。
...
图片是风格校准样例，不是构图模板。使用时应从当前文章重新发明隐喻，不要照抄它的物件和构图。
...
- Aiko 必须承担核心动作；如果去掉 Aiko 画面仍然完全成立，说明 Aiko 太装饰了。
- 示例图只用于校准线条密度、留白、颜色克制和 Aiko 参与方式，不要复刻构图。
